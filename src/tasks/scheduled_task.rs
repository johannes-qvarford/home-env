...
impl task::Task for ScheduledTask {
    ...
    fn execute(&self) -> Result<()> {
        ...
        process::execute("powershell.exe", &["-command", &powershell_string], &acceptable_status_codes).wrap_err_with(|| format!("Executing powershell string '{powershell_string}'"))
    }
}
...
pub(crate) fn scheduled_task(name: &str, at: &str) -> Box<dyn task::Task> {
    Box::new(ScheduledTask { name: name.to_owned(), at: at.to_owned() })
}