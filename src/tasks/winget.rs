...
impl task::Task for Winget{
    ...
    fn execute(&self) -> Result<()> {
        let package_name = self.package_name;
        let acceptable_status_codes = [WINGET_INSTALLER_HASH_DOES_NOT_MATCH, WINGET_FAILED_TO_INSTALL_PACKAGE, WINGET_PACKAGE_CANNOT_BE_UPGRADED];
        process::execute("winget.exe", &["install", self.package_name], &acceptable_status_codes).wrap_err(format!("Calling winget install {package_name}"))
    }
}
