...
impl dyn Task {
    pub(crate) fn execute_if_needed(&self) -> Result<Execution> {
        ...
        let executed = self.has_been_executed().wrap_err("Checking if task been executed")?;
        if !executed {
            ...
            println!("{}", &format!("Skipping task '{name}' since it has already been executed.").blue());
    ...
    fn has_been_executed(&self) -> Result<bool> {
        ...
        path.try_exists().wrap_err_with(|| format!("Verifying existance of mark path '{path:?}'"))
    }

    fn mark_executed(&self) -> Result<()> {
        File::create(self.mark_path().wrap_err("Calculating path of mark file to create")?).wrap_err("Creating mark file")?;
        Ok(())
    }
    ...
    fn mark_directory(&self) -> Result<PathBuf> {
        let mut dir = config_local_dir().ok_or(Report::msg("Config local directory was not found"))?;
