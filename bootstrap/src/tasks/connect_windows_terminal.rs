...
use crate::{utility::task::{self}, utility::symlink};
...
impl task::Task for ConnectWindowsTerminal {
    ...
    fn execute(&self) -> Result<()> {
        ...
        // TODO: SymbolicLink
        symlink::symlink_file(terminal_path.to_str().unwrap(), home_env_path.to_str().unwrap()).wrap_err_with(|| format!("Symlinking {home_env_path:?} to {terminal_path:?}"))?;
        ...
        if !status.is_success() {
            return Err(Report::msg(format!("Unsuccessful response when fetching terminal pic url: {status}")));
        }

        let bytes = pic_response.bytes().wrap_err("Converting response to bytes")?;

        let pic_dir = dirs::picture_dir().ok_or(Report::msg(format!("Could not determine picture directory.")))?;
        let mut pic_file = pic_dir.clone();
        pic_file.push("terminal_background.png");
        let mut file = File::create(pic_file).wrap_err_with(|| format!("Creating picture at '{pic_dir:?}'"))?;
        file.write_all(&bytes).wrap_err("Writing bytes to picture")?;
