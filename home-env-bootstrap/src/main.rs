...
fn run_cli_mode(args: Args, tasks: Vec<Box<dyn utility::task::Task>>) -> Result<()> {
    if let Some(task_name) = args.task {
        let task = tasks
            ...
            .wrap_err_with(|| format!("Looking for task with name {task_name}"))?;
        task.execute_or_pause()
            .wrap_err_with(|| format!("Executing task {task_name}"))?;
        task.mark_executed()
            .wrap_err_with(|| format!("Marking task {task_name} as executed"))?;
        println!(
            "{}",
            &format!("Forcefully executed task '{task_name}'.")
                .if_supports_color(Stdout, |x| x.yellow())
        );
        return Ok(());
    }
    ...
    for task in tasks {
        ...
        if skip > 0
            && !task
                .has_been_executed()
                .wrap_err_with(|| format!("checking if task {name} has been executed"))?
        {
            skip -= 1;
            task.mark_executed()
                .wrap_err_with(|| format!("marking task {name} as executed"))?;
            println!(
                "{}",
                &format!("Forcefully marked task '{name}' as executed.")
                    .if_supports_color(Stdout, |x| x.yellow())
            );
        }
